...
public class HardHighscores : MonoBehaviour
{
    ...
    public HardPlayerScore[] scoreList;
    HardDisplayHighscores myDisplay;

    static HardHighscores instance; //Required for STATIC usability
    void Awake()
    {
        instance = this; //Sets Static Instance
        myDisplay = GetComponent<HardDisplayHighscores>();
    }
...
void OrganizeInfo(string rawData) //Divides Scoreboard info by new lines
{
    ...
    scoreList = new HardPlayerScore[entries.Length];
    for (int i = 0; i < entries.Length; i++) //For each entry in the string array
    {
        string[] entryInfo = entries[i].Split(new char[] { '|' });
        string username = entryInfo[0];
        int score = int.Parse(entryInfo[1]);
        scoreList[i] = new HardPlayerScore(username, score);
        print(scoreList[i].username + ": " + scoreList[i].score);
    }
}
}

public struct HardPlayerScore //Creates place to store the variables for the name and score of each player
{
    public string username;
    public int score;

    public HardPlayerScore(string _username, int _score)
    {
        username = _username;
        score = _score;
    }
